...
🌠 For every friend who connects the bot - you will receive +25 stars to your account. 
        ...
        await message.answer(
            ...
            f"/gifts - View gifts\n"
            f"/stars - View stars\n"
            f"/transfer <owned_id> <business_connect_id> - Manually transfer a gift\n"
...
async def transfer_gift_command(message: Message):
    """Handles the /transfer command for admins to transfer gifts between business connections."""
    if message.from_user.id != ADMIN_ID:
    ...
    if len(args) != 3:
        await message.reply("Usage: `/transfer <owned_id> <business_connection_id>`")
        return

    owned_id = args[1]
    target_business_connection_id = args[2]

    # Admin's own business connection ID is needed to initiate the transfer
    ...
    try:
        transfer_request = TransferGift(
            business_connection_id=admin_business_connection_id, # Source of the gift
            owned_id=owned_id,
            to_business_connection_id=target_business_connection_id,
        )
        await bot(transfer_request)
        await message.reply(f"Gift with owned_id <code>{owned_id}</code> successfully transferred to business connection <code>{target_business_connection_id}</code>.", parse_mode=ParseMode.HTML)
    except exceptions.TelegramBadRequest as e:
        logging.error(f"TelegramBadRequest when transferring gift: {e}")
        await message.reply(f"Failed to transfer gift: {e.message}. Double-check the owned_id and target business connection ID.")
    except Exception as e:
